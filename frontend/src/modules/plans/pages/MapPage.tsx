...
export function MapPage() {
  ...
  const [addingToPlan, setAddingToPlan] = React.useState(false);
  ...
  const handleAddToPlan = useCallback(async (place: Place) => {
    if (!planId) {
      navigate('/plans');
      return;
    }
    ...
    try {
      const plan = await planService.getOne(planId);
      await planService.addSubplan(planId, {
        placeId: place.id,
        placeName: place.name,
        notes: place.address ?? undefined,
        order: plan.subplans.length,
      });
      navigate(`/plans/${planId}`, {
        state: { addedPlaceId: place.id },
      });
    } catch {
      setPlanActionError('No se pudo agregar el lugar al plan. Intenta de nuevo.');
    } finally {
  ...
  return (
    <div style={styles.page}>
      ...
      {planActionError && !loading && (
        <div style={styles.errorBadge}>{planActionError}</div>
      )}
