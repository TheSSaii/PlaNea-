...
export default function TopicDetail() {
  ...
  useEffect(() => {
    if (!username) return
    getLikes(Number(id), username).then(data => {
      setLiked(data.liked)
  ...
  const handleLike = async () => {
    ...
    try {
      const data = await toggleLike(Number(id), name)
      setLiked(data.liked)
      setLikeCount(data.count)
    } catch (e) {
  ...
  const handleComment = async () => {
    ...
    setSending(true)
    await addComment(Number(id), { content: commentText, author })
    const updated = await getTopicById(Number(id))
    setTopic(updated)
    setCommentText('')
    setAuthor('')
    setSending(false)
              ...
              {(topic.comments ?? []).map(comment => (
                <div key={comment.id} className="flex gap-3">
                  ...
                  <div className="bg-gray-50 rounded-2xl px-3 py-2 flex-1">
                    <p className="font-semibold text-xs text-gray-900 mb-1">{comment.author}</p>
