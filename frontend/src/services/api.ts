...
// Agregar comentario
export const addComment = (topicId: number, data: CreateCommentDto) =>
  api.post(`/forum/topics/${topicId}/comments`, data).then(r => r.data);

// Dar o quitar like
export const toggleLike = (topicId: number, username: string) =>
  api.post(`/forum/topics/${topicId}/like`, { username }).then(r => r.data)

// Obtener likes de un topic
export const getLikes = (topicId: number, username: string) =>
...
// Editar topic
export const updateTopic = (id: number, data: { title?: string; content?: string; author: string }) =>
