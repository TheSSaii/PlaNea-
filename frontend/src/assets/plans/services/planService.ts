...
export const planService = {
  getAll: (status?: string) =>
    api.get<RawPlan[]>('/plans', { params: status ? { status } : {} })
      .then(r => r.data.map(adaptPlan)),

  getOne: (id: string) =>
    api.get<RawPlan>(`/plans/${id}`)
  ...
  update: (id: string, payload: PlanPayload) =>
    api.patch<RawPlan>(`/plans/${id}`, adaptPayload(payload))
      .then(r => adaptPlan(r.data)),
  ...
  addSubplan: (planId: string, data: Omit<Subplan, 'id' | 'planId'>) =>
    api.post<RawSubplan>(`/plans/${planId}/subplans`, data)
